...
void Scene::addToplevel(Toplevel *c)
{
    ...
    if (c->surface()) {
        ...
        connect(c->surface(), &KWayland::Server::SurfaceInterface::scaleChanged, this, discardQuads);
    }
...
WindowQuadList Scene::Window::makeContentsQuads() const
{
    ...
    while (!stack.isEmpty()) {
        ...
        for (const QRectF &rect : region) {
            ...
            const qreal x0 = rect.left() + position.x();
            const qreal y0 = rect.top() + position.y();
            const qreal x1 = rect.right() + position.x();
            const qreal y1 = rect.bottom() + position.y();

            const qreal u0 = rect.left() * scale;
            const qreal v0 = rect.top() * scale;
            const qreal u1 = rect.right() * scale;
            const qreal v1 = rect.bottom() * scale;
