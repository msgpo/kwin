...
void OpenGLWindow::initializeRenderContext(RenderContext &context, const WindowPaintData &data)
{
    ...
    if (!renderNodes[context.shadowOffset].quads.isEmpty()) {
        ...
        renderNodes[context.shadowOffset].texture = shadow->shadowTexture();
        renderNodes[context.shadowOffset].opacity = data.opacity();
        renderNodes[context.shadowOffset].hasAlpha = true;
        renderNodes[context.shadowOffset].coordinateType = NormalizedCoordinates;
        renderNodes[context.shadowOffset].leafType = ShadowLeaf;
    }

    if (!renderNodes[context.decorationOffset].quads.isEmpty()) {
        renderNodes[context.decorationOffset].texture = getDecorationTexture();
        renderNodes[context.decorationOffset].opacity = data.opacity();
        renderNodes[context.decorationOffset].hasAlpha = true;
        renderNodes[context.decorationOffset].coordinateType = UnnormalizedCoordinates;
        renderNodes[context.decorationOffset].leafType = DecorationLeaf;
    }
    ...
    stack.push(windowPixmap<OpenGLWindowPixmap>());
    ...
    while (!stack.isEmpty()) {
        ...
        renderNodes[context.contentOffset + i].texture = windowPixmap->texture();
        renderNodes[context.contentOffset + i].hasAlpha = windowPixmap->hasAlphaChannel();
        renderNodes[context.contentOffset + i].opacity = contentOpacity;
        renderNodes[context.contentOffset + i].coordinateType = UnnormalizedCoordinates;
        renderNodes[context.contentOffset + i].leafType = ContentLeaf;
        i++;
    ...
    if (data.crossFadeProgress() != 1.0) {
        ...
        if (previous) { // TODO(vlad): Should cross-fading be disabled on Wayland?
            ...
            for (const WindowQuad &quad : qAsConst(renderNodes[context.contentOffset].quads)) {
                ...
                renderNodes[context.previousContentOffset].quads.append(newQuad);
            }

            renderNodes[context.previousContentOffset].texture = previous->texture();
            renderNodes[context.previousContentOffset].hasAlpha = previous->hasAlphaChannel();
            renderNodes[context.previousContentOffset].opacity = data.opacity() * (1.0 - data.crossFadeProgress());
            renderNodes[context.previousContentOffset].coordinateType = NormalizedCoordinates;
            renderNodes[context.previousContentOffset].leafType = PreviousContentLeaf;

            context.quadCount += renderNodes[context.previousContentOffset].quads.count();
        }
    }
}
