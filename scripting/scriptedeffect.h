...
public:
     ...
     * @param w The window.
     * @param grabRole The grab role.
     * @param force By default, if the window is grabbed another effect, then that
     *   window won't be ungrabbed. If you would like to ungrab a window even if
     *   it's grabbed by another effect, then pass @c true.
     * @returns @c true if the window was ungrabbed successfully, otherwise @c false.
     **/
    Q_SCRIPTABLE bool ungrab(KWin::EffectWindow *w, DataRole grabRole, bool force = false);
