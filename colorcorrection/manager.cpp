...
void Manager::toggle()
{
    m_isInhibited = !m_isInhibited;
    m_isInhibited ? inhibit() : uninhibit();
}

bool Manager::isInhibited() const
{
    return m_inhibitRefereceCount;
}

void Manager::inhibit()
{
    m_inhibitRefereceCount++;

    if (m_inhibitRefereceCount == 1) {
        resetAllTimers();
